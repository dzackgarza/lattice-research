...
**Pipeline state:** `just test` fails at `_mypy`. Ruff/format passes (no lint/format output).
Mypy is the sole blocker.
...
**Now (unblocked):**
1. **Close `TASK-QC-GROUND-CATEGORY-SPEC-CALLABLE-TYPES`** (needs-agent-review → done).
   This is the prerequisite before `PHASE-QC-BASIC-TYPING-HYGIENE` can mark complete.
   Covers 86 `[valid-type]` errors.
2. **Start `TASK-QC-BASIC-MYPY-HYGIENE-INVENTORY`** — largest unblocked code-gap bucket.
   Priority sub-items:
...
**Open question:** The `category_specs.refinement` attribute missing from category-obligation test (`test_spec_obligations.py:70`) — is this a missing module or a renamed subpackage?
Not tracked in any current task card.
