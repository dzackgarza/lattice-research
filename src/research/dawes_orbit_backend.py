...
def induced_discriminant_action(lattice, M):
    ...
    images = tuple(disc(M * generator.lift()) for generator in disc.gens())
    return _reduce_discriminant_action_matrix(
        lattice,
...
class _SubgroupOrbitSpec:
    ...
    def discriminant_target(self):
        if self.discriminant_subgroup is not None:
            return self.discriminant_subgroup
        return self.lattice.discriminant_group().orthogonal_group()

    def as_constraints(self):
        return {
            "determinant": self.determinant,
            "spinor": self.spinor,
            "discriminant_subgroup": self.discriminant_subgroup,
            "opaque": self.opaque,
        }
...
class _ComplementDecomposition:
    ...
    def inject_line_discriminant(self, element):
        lift = vector(
            QQ,
            list(element.lift()) + [QQ.zero()] * self.complement_rank,
        )
...
class _DawesOrbitBackend:
    ...
    def _find_relaxed_structured_witness(self, ambient_witness):
        relaxed_constraints = self._problem.spec.as_constraints()
        relaxed_constraints["determinant"] = None
        relaxed_constraints["spinor"] = None
        return _find_structured_witness_with_constraints(
            self._problem.lattice,
            self._problem.w2,
            ambient_witness,
            relaxed_constraints,
    ...
    def _solve_algorithm_23(self, left, right) -> bool:
        for line_isometry in _rank_one_discriminant_isometries(
            ...
            for complement_isometry in _search_discriminant_form_isometries(
                ...
                if discriminant_action in self._problem.spec.discriminant_target():
                    return True
        return False
...
def _build_complement_decomposition(lattice, w):
    ...
    direct_sum_lattice = type(lattice).from_sage(IntegralLattice(direct_sum_gram))
...
def _find_structured_witness(group, w2, ambient_witness, constraints):
    witness = _find_structured_witness_with_constraints(
        group.lattice,
        w2,
        ambient_witness,
        constraints,
...
def _find_structured_witness_with_constraints(
    lattice,
    w2,
    ambient_witness,
    constraints,
...
def _candidate_satisfies_constraints(
    ...
    if constraints["discriminant_subgroup"] is not None:
        ...
        if combined not in constraints["discriminant_subgroup"]:
            return False
    return True


def _structured_identity(lattice, constraints):
    image = {}
    if constraints["discriminant_subgroup"] is not None:
        discriminant_rank = len(lattice.discriminant_group().gens())
...
def _structured_image(lattice, M, constraints, *, precomputed_spinor=None):
    image = {}
    if constraints["discriminant_subgroup"] is not None:
...
def _multiply_structured_images(lattice, left, right, constraints):
    image = {}
    if constraints["discriminant_subgroup"] is not None:
...
def _orthogonal_complement_data(lattice, w):
    ...
    complement_lattice = type(lattice).from_sage(IntegralLattice(complement_gram))
