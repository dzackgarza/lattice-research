...
(`ZZ`, `QQ`, `GF(p)`, etc.) and runs the same documented examples through
the spec-wrapped call (`Rings().Constructors().X(...)`).  If anything breaks,
the spec has introduced a regression.
