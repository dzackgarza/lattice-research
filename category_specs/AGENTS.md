...
`category_specs` exists to translate Sage and exact-backend surfaces into the bounded
mathematical language required by `GOAL.md`: typed objects, constructors, Hom/End/Aut
vocabulary, modules with forms, lattices, and backend bridges that later Coble/K3 work
can use without raw-matrix folklore. Its purpose is not to maintain method inventories,
ledgers, or diagnostic classifications for their own sake, and it is not a catalog of
every Sage method.

The spec phase is source-grounded and feasibility-classified. No spec operation enters
the public API merely because it is mathematically nameable. Every admitted surface must
be Sage-backed, backend-backed by a named exact package, locally implementable as a
...
`R`-module only when Sage gives a usable parent/element/category surface and the
required witness data is recorded; define homomorphisms by images of generators when
exact linear algebra can check domain, codomain, linearity, matrix realization, and
structure preservation; construct modules with bilinear or quadratic forms where the
form data has an exact representation; and compute or check Gram matrices, orthogonal
complements, primitive sublattices, discriminant groups/forms, and base changes when
these reduce to Sage-backed or bounded local exact algebra.

Hom, End, and Aut are mathematical objects, but their names do not imply generic
computability. A candidate isometry or embedding may be certified by finite exact
checks. The subgroup generated by certified isometries may be represented from finite
data. Full automorphism-group computation is admitted only for restricted classes with
a named Sage surface, backend, or separately planned algorithmic owner.
...
hypothesis, representation split, Sage bridge point, feasibility status, backend
witness, or missing-category obligation. A spec item is in scope when it is needed to
express later lattice or Coble computations as typed mathematical constructions and has
an admission status, or when Sage investigation shows that omitting it would force raw
...
structures, documents arbitrary concrete methods, promotes a mathematically nameable
operation without feasibility evidence, or hides a global algorithmic problem inside
baseline category plumbing. Later geometry vocabulary is recorded as deferred until the
lattice substrate exists.
...
- Classify each operation by feasibility: Level 0 definition, Level 1 certification,
  Level 2 finite-data construction, Level 3 bounded or finite search, or Level 4 global
  algorithmic computation requiring separate ownership.
...
- The category-spec project exists to specify a Sage-grounded mathematical interface
  inside Sage's category/object universe. Current Sage coverage is not automatically the
  adequacy standard, but current exact feasibility is an admission constraint. Sage
  interop is still a design constraint: refined Sage objects should remain usable by
  existing Sage code when mathematically appropriate. Inventory and mapping preserve
  existing Sage functionality, bound the spec to implementable mathematics, and record
  missing mathematical methods only with an admission status: Sage-backed,
