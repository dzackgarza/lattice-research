---
id: PLAN-CATEGORY-OBLIGATION-EXAMPLES
trackerStatus:
  type: plan
parents:
...
title: Category obligation examples and failed-assertion classification
status: complete
priority: critical
owner: Zack
description: Group category-obligation examples, failed-assertion classification, and
  object-shape probe governance so they support the foundational plan instead of
  becoming a disconnected cleanup backlog.
successCriteria:
- Failed category assertions are classified by mathematical cause before they are
  routed to spec, implementation, research, or decision cards.
...
Group category-obligation examples, failed-assertion classification, and object-shape
probe governance so they support the foundational plan instead of becoming a
...
- [ ] Failed category assertions are classified by mathematical cause before they are routed to spec, implementation, research, or decision cards.
- [ ] Audit cards link to the plan or source map whose correctness they protect.
- [ ] Audit coverage includes duck-typed object-shape probes where category-spec code
  should match real Sage/project types, documented wrappers, or category membership.
...
| "Existing category-obligation example and variadic sprint plans under `plans/features/`" | Vague. Not a specific file citation. |
...
1. **Failed category assertions routed by mathematical cause** — verifiable by inspecting failed category assertion cards and checking that each links to a spec/implementation/research/decision card with a cause statement. Slightly subjective ("mathematical cause") but operationally checkable.
...
_Note:_ Criterion 1 ("by mathematical cause") could use a sharper operational definition (e.g., "each failed category assertion card's body contains a `Cause:` section citing a spec line, a Sage source line, or a decision card ID"), but is acceptable as-is.
...
**Finding 2:** `PHASE-VARIADIC-SIGNATURE-CLOSURE-AUDIT` does not exist as a directory or file under this plan's workspace (`.../PLAN-CATEGORY-OBLIGATION-EXAMPLES/`). It was found under `PLAN-SAGE-SURFACE-CONSTRUCTOR-ADMISSION/`, a sibling plan under the same feature. This means:
...
**Current phase inventory vs. declared scope:** The plan's description promises "category-obligation examples and failed assertions, audit, variadic-signature, import hygiene, wrapper, type, and anti-slop compliance work" but the only active phase is duck-type probe audit. Variadic signature audit lives under a different plan. Import hygiene, wrapper, type, and anti-slop compliance have no phases at all.

#### G4 — Scope Containment: PASS

The plan's scope is defined by:

- **Objective (line 33):** "Group category-obligation examples and failed assertions, audit, variadic-signature, import hygiene, wrapper, type, and anti-slop compliance work so it supports the foundational plan."
...
The child phase's review log confirms no spec/category-obligation weakening occurred in its tasks. The plan is additive governance, not scope reduction.
