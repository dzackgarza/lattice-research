---
id: TASK-01KQN9J3WXGKSYTRTQDP54C28J-FIX-MODULES-CATEGORY-OBLIGATION-MISSING-ALGEBRA-SYMPY-RICHCMP-REPRESENTATION-LATTICE-A
trackerStatus:
  type: task
parents:
- '[[PHASE-RING-AXIOM-Q-ADIC-AND-MATRIX-ALGEBRA-SURFACES]]'
dependsOn: []
title: Fix Modules category-obligation example missing algebra _sympy_ __richcmp__ representation lattice
  and graded base-category failures
status: complete
priority: high
description: The deleted Modules triage recorded the post-wrapper-deletion failed category assertions
...
- Run just category-obligation-file modules/category_obligations.sage and preserve the full missing-surface
  output in tracker updates.
- Do not restore constructor-only wrapper categories to make category-obligation examples pass.
tags:
- FEATURE-CATEGORY-SPECS-AND-SAGE-SURFACES
- PLAN-SAGE-SURFACE-CONSTRUCTOR-ADMISSION
- PHASE-RING-AXIOM-Q-ADIC-AND-MATRIX-ALGEBRA-SURFACES
---
# Fix Modules category-obligation example missing algebra _sympy_ __richcmp__ representation lattice and graded base-category failures
## Summary

The deleted Modules triage recorded the post-wrapper-deletion failed category assertions and the
...
- Original migrated line: `Fix Modules category-obligation example missing algebra _sympy_ __richcmp__ representation lattice and graded base-category failures from category_specs/modules/docs/TRIAGE.md`
...
- Current failed category assertions include missing algebra, _sympy_, __richcmp__, RepresentationModules KeyError, IntegerLattices/TorsionQuadraticModules compatibility KeyError, and graded module base-category mismatch.
...
- [ ] Run just category-obligation-file modules/category_obligations.sage and preserve the full missing-surface output in tracker updates.
- [ ] Do not restore constructor-only wrapper categories to make category-obligation examples pass.
...
- 2026-05-06 start-of-task category-obligation example after fork-isolated category-obligation example harness: `just
  --justfile category_specs/justfile category-obligation-file modules/category_obligations.sage` fails on
  current module frontiers. The former migrated `_sympy_` headline is no longer
  present in the filtered category-obligation output. Current repeated frontiers include
...
- 2026-05-06 constructor-refinement slice: changed module constructor refinement to
  ...
  exercised directly. Re-running `just --justfile category_specs/justfile category-obligation-file
  modules/category_obligations.sage` removed the repeated `modify_module_structure` constructor
...
- 2026-05-06 finite-rank dimension slice: implemented
  ...
  the modules category-obligation example removed the `VectorSpaceWithoutBasis(2) has dimension 2` failure;
...
- 2026-05-06 OverPID formed-chain slice: changed
  ...
  the modules category-obligation example removed the vector-space inner-product and explicit
...
- 2026-05-06 membership-refinement and join-super slice: changed direct
  membership-only category-obligation example refinements to call `refine_category(..., test=False)`, so
  ...
  category-obligation-file modules/category_obligations.sage` removed the subobject/quotient
...
- 2026-05-06 ring-constructor refinement slice: changed polynomial, power-series,
  ...
  the matrix-ring and module-constructor treatment: constructor category-obligation examples preserve the
  ...
  category_specs/justfile category-obligation-file modules/category_obligations.sage` removed the
  ring-as-module constructor failures. Remaining modules failed category assertionss are the
...
- 2026-05-06 ideal-as-submodule slice: changed ideal module constructors to realize
  ...
  category_specs/justfile category-obligation-file modules/category_obligations.sage` removed the ideal
  `_refine_category_` failures. The only remaining modules failed category assertions is the
...
- 2026-05-06 graded-module slice: added a local `Modules(R).Graded()` axiom surface
  ...
  category-obligation-file modules/category_obligations.sage` passes with no modules failed category assertions. Status
...
- Broader `rings/category_obligations.sage` still fails on known ring-frontier surfaces, but those
  failures are outside this modules category-obligation card's discharge claim and remain preserved
