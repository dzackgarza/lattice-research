...
factor of `68`.  The actual mod-2 image of `Stab_{O(S_En)}(h)` has order `696729600`,
generated by the `U` swap fixing `h` and the simple-root reflections in `E_8(-1)`;
the full finite stabilizer is larger than this image by a factor of `136`.

The same Sage/GAP witness now computes the induced finite image on
`A_TCo` for `delta^perp=Z(u+v)+U(2)+E_8(2)`.  In
`B_Co=<1>+U+E_8(-1)` coordinates, the action fixes the `<1>` coordinate and uses the
actual Enriques stabilizer image on the remaining coordinates.  The image order is
...
This is the subgroup to compare with
`Gamma_Co^En(delta)=im(Stab_{Gamma_En,2}(Z delta)->O(delta^perp))` after choosing the
source-backed isometry `T_Co ~= delta^perp`, the transported polarization condition,
and the compatibility of `gamma_Co` with the AEGS gluing anti-isometry.
