...
generated by semantic extraction from Sage method body/docs/examples and contains
...
extraction, populating semantically extracted `U_math`, moving specific `frontierId`
rows from `Remaining_math` into `C_math`, `R_math`, or `Q_math`, or performing
compatibility audit work. Mapping docs, cards, decisions, review logs, handoffs, and
commits are evidence or routing; they are not mathematical progress evidence unless the
