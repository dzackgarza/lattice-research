...
- Sage constructor/method inventory and mapping progress is controlled by
  ...
  `Remaining_math = U_math - C_math - R_math - Q_math`, where `U_math` is generated by
  semantic extraction from Sage method body/docs/examples before owner classification.
  A mathematical progress claim must name primary rows moved from `Remaining_math` into
