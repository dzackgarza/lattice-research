...
§ Before creating new scripts or tooling, always check what already exists — vault root
(/home/dzack/notes/Obsidian/), .hermes/scripts/, and ~/ai/opencode/skills/. The vault
has 8+ Python scripts at root level for atomization/OCR pipelines; .hermes/scripts/ has
graph analysis, link repair, and classification tools; ~/ai/opencode/skills/ has
...
§ Nvidia NIM API (integrate.api.nvidia.com) with NVIDIA_NIM_API_KEY for vision:
meta/llama-3.2-90b-vision-instruct reliable for math diagrams (100% content, no
reasoning field). atomize_images.py has --transcribe flag for this.
Hermes vision_analyze uses nemotron-30b via OpenRouter (~60% reliable).
Critical: inspect ONE raw response before batch, read `content` not `reasoning`, verify
field structure before coding extraction.
