...
# This file was *autogenerated* from the file computations/task1_2b_discriminant_forms.sage
from sage.all_cmdline import *   # import sage library

_sage_const_80 = Integer(80); _sage_const_2 = Integer(2); _sage_const_10 = Integer(10); _sage_const_11 = Integer(11); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_8 = Integer(8); _sage_const_4 = Integer(4); _sage_const_9 = Integer(9); _sage_const_2048 = Integer(2048); _sage_const_20 = Integer(20); _sage_const_18 = Integer(18); _sage_const_16 = Integer(16); _sage_const_6 = Integer(6); _sage_const_3 = Integer(3); _sage_const_14 = Integer(14); _sage_const_5 = Integer(5); _sage_const_60 = Integer(60); _sage_const_22 = Integer(22); _sage_const_19 = Integer(19)
...
S_gram = diagonal_matrix(QQ, [_sage_const_2 ] + [-_sage_const_2 ]*_sage_const_10 )
...
sig_S = QF_S.signature_vector()[:_sage_const_2 ]
...
expected_card = _sage_const_2 **_sage_const_11 
...
brown_S = (sig_S[_sage_const_0 ] - sig_S[_sage_const_1 ]) % _sage_const_8 
print(f"  Brown(q_S) = signature(S_Co) = {sig_S[_sage_const_0 ]} - {sig_S[_sage_const_1 ]} = {brown_S} (mod 8)")
...
def gauss_sum(A, q_gram):
    ...
    for i in range(_sage_const_2 **n):
        ...
        x = vector(QQ, [(i >> j) % _sage_const_2  for j in range(n)])
        
        # Compute q(x) = x^T * q_gram * x
        q_val = (x * q_gram * x)[_sage_const_0 ]
        ...
        q_val_mod = QQ(q_val - _sage_const_2  * floor(q_val / _sage_const_2 ))
...
brown_S_computed = round((_sage_const_4  / pi) * CC(phase).arg()) % _sage_const_8 
...
T0_gram = diagonal_matrix(QQ, [_sage_const_2 , _sage_const_2 ] + [-_sage_const_2 ]*_sage_const_9 )
...
sig_T0 = QF_T0.signature_vector()[:_sage_const_2 ]
print(f"  Signature: {sig_T0}")
print(f"  Determinant: {T0.determinant()}")

assert sig_T0 == (_sage_const_2 , _sage_const_9 ), f"Signature mismatch: {sig_T0} != (2,9)"
assert abs(T0.determinant()) == _sage_const_2048 , f"Discriminant mismatch: {abs(T0.determinant())} != 2048"
...
brown_T0 = (sig_T0[_sage_const_0 ] - sig_T0[_sage_const_1 ]) % _sage_const_8 
print(f"  Brown(q_T0) = signature(T0) = {sig_T0[_sage_const_0 ]} - {sig_T0[_sage_const_1 ]} = {brown_T0} (mod 8)")
...
brown_T0_computed = round((_sage_const_4  / pi) * CC(phase_T0).arg()) % _sage_const_8 
...
print(f"  -Brown(q_S) = {-brown_S % _sage_const_8 } (mod 8)")

# For q_T = -q_S, we need Brown(q_T) = -Brown(q_S) (mod 8)
brown_relation = (brown_T0 == (-brown_S) % _sage_const_8 )
...
n_pos_S = _sage_const_1    # one (1/2) term
n_neg_S = _sage_const_10   # ten (-1/2) terms

n_pos_T0 = _sage_const_2    # two (1/2) terms
n_neg_T0 = _sage_const_9    # nine (-1/2) terms

n_pos_negS = _sage_const_10   # ten (1/2) terms in -q_S
n_neg_negS = _sage_const_1    # one (-1/2) term in -q_S
...
print(f"    sig(-q_S) = {(-brown_S) % _sage_const_8 } (mod 8)")
...
# E8 root system
R_E8 = RootSystem(['E', _sage_const_8 ])
L = R_E8.root_lattice()
...
print(f"  E8 total roots: {_sage_const_2  * len(positive_roots)}")
...
def root_norm(root_vec):
    ...
    return (root_vec * E8_cartan * root_vec)[_sage_const_0 ]

def roots_orthogonal(r1, r2):
    """Check if two roots are orthogonal."""
    return (r1 * E8_cartan * r2)[_sage_const_0 ] == _sage_const_0 
...
assert all(n == _sage_const_2  for n in norms), f"Not all roots have norm 2: {unique_norms}"
...
def find_maximal_orthogonal_set(roots):
    ...
    for i, r in enumerate(roots):
        orth = [r]
        for j in range(i+_sage_const_1 , len(roots)):
...
assert len(orth_roots) == _sage_const_8 , f"Expected 8 orthogonal roots, got {len(orth_roots)}"
...
for i in range(len(orth_roots)):
    for j in range(i+_sage_const_1 , len(orth_roots)):
...
U_gram = matrix(QQ, [[_sage_const_0 , _sage_const_1 ], [_sage_const_1 , _sage_const_0 ]])
...
print(f"  Signature: {QuadraticForm(Lambda_gram).signature_vector()[:_sage_const_2 ]}")
...
# e0 = (1, 1) in U1, norm = 2
v0 = vector(QQ, [_sage_const_1 , _sage_const_1 ] + [_sage_const_0 ]*_sage_const_20 )
embed_S.append(v0)

# e1 = (1, -1) in U1, norm = -2
v1 = vector(QQ, [_sage_const_1 , -_sage_const_1 ] + [_sage_const_0 ]*_sage_const_20 )
embed_S.append(v1)

# e2 = (1, -1) in U2, norm = -2
v2 = vector(QQ, [_sage_const_0 , _sage_const_0 , _sage_const_1 , -_sage_const_1 ] + [_sage_const_0 ]*_sage_const_18 )
embed_S.append(v2)

# e3 = (1, -1) in U3, norm = -2
v3 = vector(QQ, [_sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_1 , -_sage_const_1 ] + [_sage_const_0 ]*_sage_const_16 )
embed_S.append(v3)
...
for i in range(_sage_const_4 ):
    ...
    v = vector(QQ, [_sage_const_0 ]*_sage_const_6  + list(orth_roots[i]) + [_sage_const_0 ]*_sage_const_8 )
    embed_S.append(v)

for i in range(_sage_const_3 ):
    # Embed in second E8(-1) at positions 14-21
    v = vector(QQ, [_sage_const_0 ]*_sage_const_14  + list(orth_roots[i]))
    embed_S.append(v)

print(f"  Total embedding vectors: {len(embed_S)}")
assert len(embed_S) == _sage_const_11 
...
off_diag_zero = all(computed_gram[i,j] == _sage_const_0  for i in range(_sage_const_11 ) for j in range(_sage_const_11 ) if i != j)
...
print(q_T_gram[:_sage_const_5 , :_sage_const_5 ])

print("\n[5.4] Computing Brown invariant of q_T...")

brown_T = (sig_T[_sage_const_0 ] - sig_T[_sage_const_1 ]) % _sage_const_8 
print(f"  Brown(q_T) = {sig_T[_sage_const_0 ]} - {sig_T[_sage_const_1 ]} = {brown_T} (mod 8)")
...
brown_relation_T = (brown_T == (-brown_S) % _sage_const_8 )
...
print(f"{'Lambda_K3':<12} {_sage_const_22 :<6} {'(_sage_const_3 , _sage_const_19 )':<12} {'_sage_const_1  (unimodular)':<15}")
...
checks = [
    ("S_Co signature (1, 10)", sig_S == (_sage_const_1 , _sage_const_10 )),
    ("T0 signature (2, 9)", sig_T0 == (_sage_const_2 , _sage_const_9 )),
    ("T_Co signature (2, 9)", sig_T == (_sage_const_2 , _sage_const_9 )),
    ("|det(S_Co)| = 2^11", abs(S_Co.determinant()) == _sage_const_2048 ),
    ("|det(T0)| = 2^11", abs(T0.determinant()) == _sage_const_2048 ),
    ("|A_S| = 2^11", A_S.cardinality() == _sage_const_2048 ),
    ("|A_T0| = 2^11", A_T0.cardinality() == _sage_const_2048 ),
