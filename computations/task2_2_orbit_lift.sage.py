...
# This file was *autogenerated* from the file computations/task2_2_orbit_lift.sage
from sage.all_cmdline import *   # import sage library

_sage_const_80 = Integer(80); _sage_const_2 = Integer(2); _sage_const_11 = Integer(11); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_5 = Integer(5); _sage_const_3 = Integer(3); _sage_const_7 = Integer(7); _sage_const_527 = Integer(527)
...
zero_rep = [_sage_const_0 ] * n_gens
print(f"\nOrbit 0 (zero vector):")
print(f"  Representative: {zero_rep}")

# Nonzero isotropic orbit representative
nonzero_rep = [_sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_1 , _sage_const_1 , _sage_const_1 , _sage_const_1 ]
...
assert zero_elem.q() == _sage_const_0 , "Zero vector should be isotropic"
assert nonzero_elem.q() == _sage_const_0 , "Nonzero representative should be isotropic"
...
all_even = all(e % _sage_const_2  == _sage_const_0  for e in diag_entries)
...
v_div2 = vector(ZZ, [_sage_const_2 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_1 , _sage_const_1 , _sage_const_1 , _sage_const_1 ])
...
assert v_div2_norm == _sage_const_0 , "Vector should be isotropic"
...
assert v2_gcd == _sage_const_1 , "Vector should be primitive"
...
assert div_v2 == _sage_const_2 , f"Expected div(v) = 2, got {div_v2}"
...
def find_isotropic_vectors_with_div(T_gram, target_div, max_count=_sage_const_5 ):
    """
    ...
    for v0 in range(-bound, bound+_sage_const_1 ):
        for v1 in range(-bound, bound+_sage_const_1 ):
            for v2 in range(-bound, bound+_sage_const_1 ):
                for v3 in range(-bound, bound+_sage_const_1 ):
                    # Try simple vectors with zeros in remaining positions
                    v = vector(ZZ, [v0, v1, v2, v3] + [_sage_const_0 ]*_sage_const_7 )
                    
                    # Check isotropy
                    if v * T_gram * v != _sage_const_0 :
                        continue
                    
                    # Check primitivity
                    if reduce(lambda a,b: ZZ.gcd(a,b), list(v)) != _sage_const_1 :
                        continue
...
div2_examples = find_isotropic_vectors_with_div(T_Co_gram, _sage_const_2 , max_count=_sage_const_5 )
for i, v in enumerate(div2_examples):
    print(f"    Example {i+_sage_const_1 }: {list(v)}")
...
for i, v in enumerate(div2_examples):
    ...
    a_t_rep = [int(c) % _sage_const_2  for c in v]
    print(f"    Example {i+_sage_const_1 }: image in A_T = {a_t_rep}")
    
    # Verify it's isotropic in A_T
    elem = sum(c * g for c, g in zip(a_t_rep, A_T.gens()) if c)
    q_val = elem.q()
    print(f"               q_T(image) = {q_val}")
    assert q_val == _sage_const_0 , f"Image should be isotropic"
...
num_nonzero_isotropic = _sage_const_527   # From Task 2.1
...
with open(output_file, 'w') as f:
    ...
    for i, v in enumerate(div2_examples):
        f.write(f"    {i+_sage_const_1 }. {list(v)}\n")
    f.write("\n")
