...
def construct_rational_sextic():
    ...
    # Step 2: Choose 9 distinct points in P^2 (start with 9)
    ...
    if len(C_sing) != 9:
        print(f"WARNING: Expected 9 singular points, found {len(C_sing)}")
        print("Singular points:", C_sing)
    else:
        print("  Correct: exactly 9 singular points")

    # Step 7: Verify no cubic passes through all 9 points
    print("\nChecking no cubic passes through all 9 points...")
    ...
    if kernel_dim == 0:
        print("  Confirmed: no cubic passes through all 9 points")
    else:
        print(f"WARNING: {kernel_dim}-dimensional space of cubics through all 9 points")
