...
# This file was *autogenerated* from the file computations/task1_2_gram_matrices.sage
from sage.all_cmdline import *   # import sage library

_sage_const_80 = Integer(80); _sage_const_11 = Integer(11); _sage_const_2 = Integer(2); _sage_const_10 = Integer(10); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_8 = Integer(8); _sage_const_3 = Integer(3); _sage_const_19 = Integer(19); _sage_const_9 = Integer(9); _sage_const_22 = Integer(22)
...
n_S = _sage_const_11   # rank of S_Co
...
sig = (sig_vec[_sage_const_0 ], sig_vec[_sage_const_1 ])
print(f"  Signature: {sig} (positive={sig_vec[_sage_const_0 ]}, negative={sig_vec[_sage_const_1 ]})")

# Verify it matches expected (1, 10)
assert sig == (_sage_const_1 , _sage_const_10 ), f"Expected signature (1,10), got {sig}"
...
print(f"  Is even: {all(S_gram[i,i] % _sage_const_2  == _sage_const_0  for i in range(S_gram.nrows()))}")
...
has_half_integer = any(val.denominator() == _sage_const_2  for val in q_S_gram.diagonal())
delta_S = _sage_const_1  if has_half_integer else _sage_const_0 

print(f"\n  δ invariant: δ_S = {delta_S}")
if delta_S == _sage_const_1 :
...
print(f"  Signature: {QF_U.signature_vector()[:_sage_const_2 ]}")
...
print(f"  Gram matrix signature: {QF_E8_neg.signature_vector()[:_sage_const_2 ]}")
...
sig_Lambda = (sig_Lambda_vec[_sage_const_0 ], sig_Lambda_vec[_sage_const_1 ])
...
assert sig_Lambda == (_sage_const_3 , _sage_const_19 ), f"Expected (3,19), got {sig_Lambda}"
...
expected_T_signature = (_sage_const_2 , _sage_const_9 )
...
T_Co_gram = diagonal_matrix(QQ, [_sage_const_2 , _sage_const_2 ] + [-_sage_const_2 ]*_sage_const_9 )
...
sig_T = (sig_T_vec[_sage_const_0 ], sig_T_vec[_sage_const_1 ])
print(f"  Signature: {sig_T}")
print(f"  Determinant: {T_Co.determinant()}")
print(f"  Is even: {all(T_Co_gram[i,i] % _sage_const_2  == _sage_const_0  for i in range(T_Co_gram.nrows()))}")
...
assert A_T.cardinality() == _sage_const_2 **_sage_const_11 , f"Expected 2^11, got {A_T.cardinality()}"
...
# For S_Co
r_S = S_Co.rank()
a_S = A_S.rank() if hasattr(A_S, 'rank') else _sage_const_11   # rank of (ℤ/2ℤ)^11
delta_S = _sage_const_1   # q_S is not integer-valued mod 2
...
r_T = T_Co.rank()
a_T = A_T.ngens()
delta_T = _sage_const_1   # q_T = -q_S, so also not integer-valued
...
print(f"{'Λ_K3':<15} {Lambda_K3.rank():<10} {'_sage_const_0  (unimodular)':<15} {'_sage_const_0 ':<5} {str(sig_Lambda):<15}")
...
verifications = [
    ...
    ("S_Co signature is (1, 10)", sig == (_sage_const_1 , _sage_const_10 )),
    ("T_Co signature is (2, 9)", sig_T == (_sage_const_2 , _sage_const_9 )),
    ("Both have discriminant (ℤ/2ℤ)^11", A_S.cardinality() == A_T.cardinality() == _sage_const_2 **_sage_const_11 ),
    ...
    ("rank(S_Co) + rank(T_Co) = 22", r_S + r_T == _sage_const_22 ),
]
