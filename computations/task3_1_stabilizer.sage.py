...
# This file was *autogenerated* from the file computations/task3_1_stabilizer.sage
from sage.all_cmdline import *   # import sage library

_sage_const_80 = Integer(80); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_8 = Integer(8); _sage_const_10 = Integer(10); _sage_const_11 = Integer(11)
import sys
...
print(f"  Diagonal: {[_sage_const_1 , _sage_const_1 ] + [-_sage_const_1 ]*_sage_const_8 }")
print(f"  θ² = I: {(theta * theta) == identity_matrix(ZZ, _sage_const_10 )}")
...
assert (theta * theta) == identity_matrix(ZZ, _sage_const_10 ), "θ should be an involution"
...
for i in range(_sage_const_2 ):
    for sign in [-_sage_const_1 , _sage_const_1 ]:
        v = vector(ZZ, [_sage_const_0 ]*_sage_const_10 )
        v[i] = sign
        roots.append(v)
...
for i in range(_sage_const_2 , _sage_const_10 ):
    for s0 in [-_sage_const_1 , _sage_const_1 ]:
        for s1 in [-_sage_const_1 , _sage_const_1 ]:
            ...
            v[i] = _sage_const_1 
            norm = v * T_En_gram * v
            if norm in [_sage_const_2 , -_sage_const_2 ]:
                if v not in roots and -v not in roots:
                    roots.append(v)

# More complex: (1, 0, 1, 1, 0, ...): 2 - 2 - 2 = -2 (norm -2)
for i in range(_sage_const_2 , _sage_const_10 ):
    for j in range(i+_sage_const_1 , _sage_const_10 ):
        for s0 in [-_sage_const_1 , _sage_const_1 ]:
            v = vector(ZZ, [_sage_const_0 ]*_sage_const_10 )
            v[_sage_const_0 ] = s0
            v[i] = _sage_const_1 
...
if len(gamma_gens) == _sage_const_0 :
    ...
    for i, g in enumerate(gamma_gens[:_sage_const_10 ]):  # Show first 10
        det = g.determinant()
        tr = g.trace()
        ...
        for k in range(_sage_const_1 , _sage_const_11 ):
            if g_power == identity_matrix(ZZ, _sage_const_10 ):
                order = k
                break
            g_power = g_power * g
        
        print(f"  Generator {i+_sage_const_1 }: det={det}, trace={tr}, order={order}")
    
    if len(gamma_gens) > _sage_const_10 :
        print(f"  ... and {len(gamma_gens) - _sage_const_10 } more generators")
...
for i, g in enumerate(gamma_gens):
    ...
    if not (stab_ok and cent_ok and iso_ok):
        print(f"  Generator {i+_sage_const_1 }: FAILED")
        all_valid = False

if all_valid and len(gamma_gens) > _sage_const_0 :
    print(f"  ✓ All {len(gamma_gens)} generators satisfy Γ_Co conditions")
elif len(gamma_gens) == _sage_const_0 :
...
with open(output_file, 'w') as f:
    ...
    f.write(f"Diagonal entries: {[_sage_const_1 , _sage_const_1 ] + [-_sage_const_1 ]*_sage_const_8 }\n")
    f.write(f"θ² = I: {(theta * theta) == identity_matrix(ZZ, _sage_const_10 )}\n\n")
    ...
    if len(gamma_gens) > _sage_const_0 :
        for i, g in enumerate(gamma_gens):
            f.write(f"Generator {i+_sage_const_1 }:\n")
