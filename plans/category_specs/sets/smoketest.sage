...
SMOKE_STATEMENTS = (
    ...
    ("ConditionSet(ZZ, even) is a subobject", lambda _: C.ConditionSet(ZZ, [lambda n: n % 2 == 0]) in Sets().Subobjects()),
    ("2 lies in ConditionSet(ZZ, even)", lambda _: 2 in C.ConditionSet(ZZ, [lambda n: n % 2 == 0])),
    ("3 does not lie in ConditionSet(ZZ, even)", lambda _: 3 not in C.ConditionSet(ZZ, [lambda n: n % 2 == 0])),
    ("ConditionSet(ZZ, even) has ambient ZZ", lambda _: C.ConditionSet(ZZ, [lambda n: n % 2 == 0]).ambient() == ZZ),
