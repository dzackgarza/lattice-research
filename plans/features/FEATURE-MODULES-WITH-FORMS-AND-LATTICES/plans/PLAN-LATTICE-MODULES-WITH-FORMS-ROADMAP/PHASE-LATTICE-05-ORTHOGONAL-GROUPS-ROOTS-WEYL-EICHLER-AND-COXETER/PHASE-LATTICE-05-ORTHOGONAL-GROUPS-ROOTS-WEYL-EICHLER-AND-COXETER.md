---
id: PHASE-LATTICE-05-ORTHOGONAL-GROUPS-ROOTS-WEYL-EICHLER-AND-COXETER
trackerStatus:
  type: phase
parents:
...
status: unstarted
priority: high
description: 'Migrated source: this plan contains the full content formerly stored at `plans/PHASE_5_ORTHOGONAL_GROUPS.md`.
  The old `plans/` copy was removed so this tracked plan is the active planning document.'
successCriteria:
- Phase 5 orthogonal groups roots Weyl Eichler and Coxeter layer child cards are complete,
...
`theory/backends/indefinite-jl.md`, and `theory/references/index.md`.

- The orthogonal group of a formed module `(M, b)` is `Aut(M, b)`. Phase 5 therefore
  treats `O(L)` as a morphism-valued parent whose elements are automorphisms preserving
  the form; matrices are only constructor inputs checked inside the group containment
  surface.
- Stabilizers, centralizers, line stabilizers, special orthogonal subgroups, and
  discriminant kernels are subgroup constructions on `O(L)` or on subgroups of `O(L)`.
  They compose centralized predicates through `ConditionSet` operators rather than
  reintroducing ad hoc matrix tests on lattice objects.
- Roots/reflections live in the free symmetric integral setting. The local mapping
  places `is_root(v)` on lattice elements and `reflection(v)` at the free symmetric
  nondegenerate level, with formula
  `s_v(w) = w - 2 b(v,w) / b(v,v) * v`; the norm convention used by the phase is
  `b(v,v) in {-2, 2}`.
- The Weyl/reflection group is the subgroup of `O(L)` generated by reflections in
  roots. Group ownership is local; computing simple roots or hyperbolic Coxeter data is
  not.
- Exact backend routing:
  indefinite lattice isometry and automorphism tasks route to Indefinite.jl
  (`INDEF_FORM_TestEquivalence`, `INDEF_FORM_AutomorphismGroup`);
  isotropic line/plane/flag orbit tasks route to Indefinite.jl
  (`INDEF_FORM_GetOrbitRepresentative`,
  `INDEF_FORM_GetOrbit_IsotropicKplane`,
  `INDEF_FORM_GetOrbit_IsotropicKflag`);
  finite group action enumeration or stabilizer search routes to GAP;
  positive-definite finite matrix-group auxiliaries may use CARAT;
  hyperbolic simple-root and Coxeter-matrix production routes to the Vinberg backend
  family described in `theory/backends/vinberg-algorithm.md`.
- The migrated phase prose records an Eichler transvection formula, but the required
  theory/reference sources in this repo do not yet pin a primary literature source for
  that formula. Child work may implement Eichler semantics only after the formula,
  hypotheses, and identities are source-mined from a cited local authority.

## Admitted Definitions

Phase 5 child work may use these source-backed definitions:

- The orthogonal group of a formed module `(M,b)` is `Aut(M,b)` in the category of
  modules with forms. Its elements are automorphisms that preserve the form. Freeness,
  nondegeneracy, and integrality are realization or algorithm hypotheses, not the
  definition. Source: `category_specs/lattices/docs/MAPPING.md`.
- Stabilizers, centralizers, discriminant kernels, and line stabilizers are subgroups
  or condition-defined subsets of an orthogonal group/Hom/Aut parent. Public ownership
  is on the group/Hom/Aut object, not on lattice helper functions. Sources:
  `.agents/skills/lattice-redesign/references/lattice-interface-style-guide.md` and
  `category_specs/homsets/docs/MAPPING.md`.
- Isometry predicates must be centralized. The matrix equation `M^T G M == G` is an
  implementation predicate inside the isometry/orthogonal-group containment surface,
  not a repeated public proof. Source:
  `.agents/skills/lattice-redesign/references/lattice-interface-style-guide.md`.
- Roots and reflections require a formed free symmetric context; the local mapping
  currently places `reflection(v)` at `Free + Symmetric + Nondegenerate` and
  `is_root(v)` at `Free + Symmetric + Integral`. Source:
  `category_specs/lattices/docs/MAPPING.md`.
- Isotropic orbit, Weyl, Coxeter, Eichler, primitive-embedding, and Nikulin/Sterk
  claims are not fully admitted by this plan alone. They require literature-backed
  hypotheses from `theory/references/index.md` and backend routing from
  `theory/backends/software-capability-map.md` before implementation or claim
  promotion.
