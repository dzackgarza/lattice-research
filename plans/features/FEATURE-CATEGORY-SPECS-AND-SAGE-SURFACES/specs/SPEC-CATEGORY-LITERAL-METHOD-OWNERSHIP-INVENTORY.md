...
| `PicardeLattice.underlying_picard_group()` | rejected malformed source spelling; do not expose this literal public API. Corrected bridge spelling, if admitted, is `PicardLattice.underlying_picard_group()` returning a `PicardGroup`. | Source-map coverage only; no implementation permission. | Source: abstract map and `[[DECISION-CATEGORY-METHOD-INVENTORY-MALFORMED-BACKEND-SURFACES]]`. Corrected bridge remains blocked by `[[DECISION-CATEGORY-METHOD-INVENTORY-PICARD-GROUP-LATTICE-OWNER]]`. |
