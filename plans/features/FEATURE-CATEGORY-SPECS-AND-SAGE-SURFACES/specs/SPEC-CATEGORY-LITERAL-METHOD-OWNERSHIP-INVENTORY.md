...
| `has_standard_involution()` | parent/interoperability predicate | decision-needed owner, likely quaternion or involutive algebra refinement | Sage exposes the name on `Algebras.ParentMethods`, but the checked source evidence is quaternion-specific and basis-dependent. Do not admit it as a generally grounded method on all `Algebras(R)`. Codomain, once admitted, would be `bool`. | Decision-needed. Source: algebras source reconciliation; algebras mapping. Decision: `[[DECISION-ALGEBRA-STANDARD-INVOLUTION-OWNER]]`. |
