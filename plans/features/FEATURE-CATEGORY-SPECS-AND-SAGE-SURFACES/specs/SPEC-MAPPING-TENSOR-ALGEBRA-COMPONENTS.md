...
| --- | --- | --- |
| `sym=` / `antisym=` on `M.tensor(...)` and `M.tensor_module(...)` | Admitted only as constructor metadata on `TensorAlgebraComponents(R).Constructors().tensor(...)` and `.component_module(...)`. No new symmetry or antisymmetry subcategory, predicate, or standalone storage object is introduced here. | Owner: constructor surface of `TensorAlgebraComponents(R)`. Hypotheses: `tensor_type() == (p,q)` and Sage position numbering on the `p+q` tensor arguments. Codomain: a tensor element or component parent with the same `tensor_type()`. Migration consequence: old code that treated symmetry data as its own public container or subtree is retired; callers pass `sym=` / `antisym=` only at construction time. |
| `Components`, `comp(...)`, `set_comp(...)`, `add_comp(...)`, `[:]`, indexed basis assignment | Remains private coordinate interop and storage. The admitted public routes are the named tensor constructors and `Tensor.structure_constants()` for type `(1,2)` product tensors. | Owner: no new public owner beyond constructor interop. Codomain consequence: no typed finite collection return is admitted for general tensor storage. Migration consequence: old component-container workflows must convert coordinate data at the boundary into a `Tensor`; they do not persist as public tensor objects. |
...
| `t.contract(...)` with Sage defaulted position conventions | Admitted only in one closed spelling: `contract(left_position, other, right_position)`. The Sage defaulted overloads such as `a.contract(b)` and `a.contract(b, 0)` are not project API. | Owner: `Tensor` elements. Hypotheses: `self` and `other` lie over the same base module, and the chosen positions form one contravariant/covariant pair across the two tensors. Codomain: `RingElement` exactly when the remaining tensor type is `(0,0)`; otherwise a tensor in the corresponding tensor component on the same base module. Migration consequence: old Sage positional shorthands and Einstein-product syntax must be rewritten to the explicit three-argument contraction call. |
