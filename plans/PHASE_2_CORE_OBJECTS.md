...
**Category/concrete split:** The category defines the MINIMUM interface
any implementation of `BilinearModules` must provide -- the mathematical
axioms. Ask: "If someone implemented this from scratch with different
internals, what must they implement?" That goes in the category. Everything
else -- signature, is_even, discriminant_group, roots, etc. -- goes on
the concrete subclasses. See "Category Interface vs. Concrete
Implementation" in `LATTICE_STYLE_GUIDE.md`.
...
class BilinearModules(Category_module):
    ...
    class ParentMethods:
        # Axiomatic interface -- what defines a BilinearModule
        def gram_matrix(self): ...        # the Gram matrix
        def bilinear_form(self): ...      # the BilinearForm object
        def codomain(self): ...           # the FormCodomain
        def dual(self): ...               # dual (axiomatically universal)
    ...
    class ElementMethods:
        # Axiomatic element operations: follow from the bilinear form axioms.
        # These apply to ANY implementation of BilinearModules.
        def bilinear_product_with(self, other): ...  # beta(v, w)
        def __mul__(self, other): ...   # v * w = beta(v, w) when v, w in same parent
        def q(self): ...                # beta(v, v)
        def is_isotropic(self): ...     # q(v) == 0
        def to_vector(self): ...        # extract coordinate vector (basis-dep)
        def span(self): ...             # sublattice generated by self
        def perp(self): ...             # orthogonal complement of span
        # NOT here: is_root, reflection, divisibility, discriminant_class
        # -- these are Lattice-specific, live on LatticeElement

    class MorphismMethods:
        # Categorical: follow from the universal properties
        def is_isometry(self): ...      # f in Hom(M,N) checks this
        # NOT here: is_involution, reflection_decomposition
        # -- these are LatticeMorphism-specific
    ...
    class Homsets(HomsetsCategory):
        ...
        class ParentMethods:
            # Standard Hom(M,N) constructors: axiomatic, any implementation needs them
