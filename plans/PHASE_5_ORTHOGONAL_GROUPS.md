...
`O(L).__call__` or `element_from_matrix`), stabilizers live on the group
...
class LatticeOrthogonalGroup(Parent):
    ...
    from matrices goes through element_from_matrix, which
    ...
    def element_from_matrix(self, M):
        ...
        """
        f = self._lattice.End().element_from_matrix(M)
    ...
    def element_from_dict(self, mapping):
        ...
        f = self._lattice.End().element_from_dict(mapping)
...
assert O_U2.element_from_matrix(swap) in O_U2
```
...
def reflection(self):
    ...
    return L.O().element_from_dict(images)
```
...
f_swap = O_U.element_from_matrix(swap)
