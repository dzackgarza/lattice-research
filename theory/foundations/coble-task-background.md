...
**Setup.** The Coble polarization $h_{\mathrm{Co}}$ is an algebraic class in
$S_{\mathrm{Co}}$. Surgery-vector computations that compare it with period-lattice or
...
**Surgery vector.** The expression $l_i = h_{\mathrm{Co}}\cdot\alpha_i$ is meaningful
only after the parents of both $h_{\mathrm{Co}}$ and the roots $\alpha_i$ have been
typed and a common pairing has been recorded. Do not infer $l=0$ from an informal
orthogonal direct-sum presentation.
