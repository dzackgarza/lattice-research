...
singleton.  Promoting this to a lattice-level $\Gamma_{\mathrm{En},2}$ root-orbit
theorem should cite the non-isotropic orbit criterion being used, for example the
Dawes/Nikulin discriminant-gluing criterion for $O_{\mathcal A}(L)$ with
$L=T_{\mathrm{En}}$ and $\mathcal A=\operatorname{im}(\Gamma_{\mathrm{En},2}\to
O(A_{T_{\mathrm{En}}}))$.
...
- record the lattice-level theorem promoting the finite double-coset calculation above
  ...
  modulo the full group $\Gamma_{\mathrm{En}}=O(T_{\mathrm{En}})$, while the finite
  discriminant split for the preimage subgroup is now computed;
