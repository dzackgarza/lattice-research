...
Detailed implementation notes, backend manuals, algorithm plans, and preserved spec backups live as memories in the iwe2 vault. Use `iwe2 search --scope project "<topic>"` or `iwe2 inspect tree --scope project` from the repo root to navigate them.
...
Before using a visible theory file as authority, identify its role. Source authority comes from `references/`; iwe2 vault memories explain how agents should compute or implement, but they do not replace literature-backed claims.
