...
Let $L$ be a lattice. The **reflection** in a vector $\alpha \in L$ with $\alpha^2 \neq 0$ is the linear transformation
$$w_\alpha(v) = v - 2\frac{v \cdot \alpha}{\alpha^2}\alpha.$$
This is an isometry of $L \otimes \mathbb{Q}$ that preserves $L$ when $\alpha$ is a root.
