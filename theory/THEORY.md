...
**OSCAR/Hecke sign convention (code boundary):** `root_lattice(:E, 8)` in OSCAR/Hecke returns the **positive definite** Cartan matrix (diagonal entries $+2$, off-diagonal $-1$). This is the standard Dynkin/Lie-algebra convention. When interfacing with OSCAR, apply a global sign flip: our $E_8$ (negative definite) corresponds to `root_lattice(:E, 8, -1)` or equivalently `rescale(root_lattice(:E, 8), -1)`. Any code that calls `root_lattice` must account for this sign difference.
...
- $\mathrm{SO}(L)$: the special orthogonal group, the index-2 subgroup of $O(L)$ of determinant-1 isometries. $\mathrm{SO}^+(L) = O^+(L) \cap \mathrm{SO}(L)$; $\widetilde{\mathrm{SO}}^+(L) = O^+(L) \cap O^*(L) \cap \mathrm{SO}(L)$.
...
Two lattices $L_1, L_2$ are in the same **genus** if $L_1 \otimes \mathbb{A} \cong L_2 \otimes \mathbb{A}$, where $\mathbb{A} = \mathbb{R} \times \prod_p \mathbb{Z}_p$ is the adele ring. Equivalently, $L_1 \otimes \mathbb{Z}_p \cong L_2 \otimes \mathbb{Z}_p$ for all primes $p$ and $L_1 \otimes \mathbb{R} \cong L_2 \otimes \mathbb{R}$.
...
The **class number** of a genus is the number of isometry classes of lattices it contains, denoted $h$. A genus with class number 1 contains a unique isometry class; in this case the genus and isometry class coincide.
...
Let $f$ be an $n$-dimensional positive definite integral quadratic form (lattice), and let $f^{(1)} = f, \ldots, f^{(h)}$ be the integrally inequivalent forms in the genus of $f$. The **mass** of the genus is:
$$m(f) = \sum_{i=1}^{h} \frac{1}{|\mathrm{Aut}(f^{(i)})|}.$$
The **mass formula** (for $n \geq 2$) expresses $m(f)$ explicitly in terms of local data:
$$m(f) = 2\pi^{-\frac{1}{4}n(n+1)} \cdot \prod_{j=1}^{n} \Gamma\!\left(\tfrac{j}{2}\right) \cdot \prod_{p} \bigl(2m_p(f)\bigr),$$
where the product over $p$ runs over all primes, and $m_p(f)$ is the **$p$-mass** of $f$, a local factor computed from the $p$-adic Jordan decomposition of $f$ (see [@conway1988mass, §4–6] for the explicit formula via diagonal factors $M_p(f_q)$, cross terms, and type factors).
...
For odd unimodular (Type I) lattices of dimension $n = 2k$:
$$\sum_{\Lambda} \frac{1}{|\mathrm{Aut}(\Lambda)|} = \frac{|B_k| \cdot |B_2 B_4 \cdots B_{2k-2}|}{2 \cdot k!} \quad (n \equiv 0 \pmod{8}),$$
with analogous formulas for other residues mod 8 involving Bernoulli numbers $B_k$ and Euler numbers $E_k$.

For even unimodular (Type II) lattices of dimension $n = 2k \equiv 0 \pmod{8}$:
$$\sum_{\Lambda} \frac{1}{|\mathrm{Aut}(\Lambda)|} = \frac{|B_k|}{2k} \prod_{j=1}^{k-1} \frac{|B_{2j}|}{4j}.$$
...
> **Source**: Standard notion; see [@Huy16, §10.3.4] for hyperkähler manifolds. Alexeev uses the correspondence with isotropic vectors without defining Lagrangian fibrations.
...
A non-zero vector $x \in L$ is **primitive** if the natural embedding $\iota_x: \langle x \rangle \hookrightarrow L$ is a primitive embedding, i.e., $L/\langle x \rangle$ is torsion-free. Equivalently, $x \neq ky'$ for any $y' \in L$ and integer $k > 1$.
...
- **Isomorphic embeddings**: Two primitive embeddings $i_1: S \hookrightarrow M_1$ and $i_2: S \hookrightarrow M_2$ are **isomorphic as embeddings** if there exists an isometry $\varphi: M_1 \xrightarrow{\sim} M_2$ such that $\varphi \circ i_1 = i_2$, i.e., $\varphi$ restricts to the *identity* on $S$. This is the finer equivalence relation.
...
**Construction via primitive extensions**: A primitive embedding $S \hookrightarrow L$ with orthogonal complement $N = S^\perp \subset L$ gives a primitive extension $S \oplus N \subseteq L$. Conversely, classifying primitive embeddings reduces to classifying gluings between anti-isometric subgroups of the discriminant groups $D_S$ and $D_N$, via Nikulin's Proposition 1.5.1.
...
A **reflection** in $O(L)$ is an involution $s \in O(L)$ whose $(-1)$-eigenspace is a rank-1 sublattice $\langle v \rangle \subset L \otimes \mathbb{Q}$, so $s$ acts as $x \mapsto -x$ on $v$ and fixes $v^\perp$ pointwise. This is reflection about the hyperplane $v^\perp$.

The explicit formula for the reflection associated to a non-isotropic vector $v$ is:
$$s_v(x) = x - \frac{2(x \cdot v)}{v^2} v.$$
A priori $s_v \in O(L \otimes \mathbb{Q})$; it lies in $O(L)$ if and only if $\frac{2(x \cdot v)}{v^2} \in \mathbb{Z}$ for all $x \in L$, i.e., $\frac{2\,\mathrm{div}(v)}{v^2} \in \mathbb{Z}$.
...
Let $L$ be a lattice. A **root** of $L$ is a vector $v \in L$ with $v^2 \neq 0$ such that the reflection $s_v(x) = x - \frac{2(x \cdot v)}{v^2}v$, a priori defined over $L \otimes \mathbb{Q}$, actually lies in $O(L)$. The integrality condition is $\frac{2\,\mathrm{div}(v)}{v^2} \in \mathbb{Z}$.

Note that $v$ and $-v$ both generate the same rank-1 sublattice and define the same reflection $s_v = s_{-v}$, so a root is really a property of the sublattice $\langle v \rangle$. By convention we choose the generator with $v^2 < 0$ (appropriate for negative-definite and hyperbolic lattices in our setting).

The norm-and-divisibility characterization of roots is in general only a sufficient condition. For even two-elementary lattices it is both necessary and sufficient:
...
The **special orthogonal group** $\mathrm{SO}(L)$ is the index-2 subgroup of $O(L)$ consisting of isometries of determinant $+1$:
$$\mathrm{SO}(L) = \ker\bigl(\det: O(L) \to \{\pm 1\}\bigr).$$
One similarly defines $\mathrm{SO}^+(L) = O^+(L) \cap \mathrm{SO}(L)$ and $\widetilde{\mathrm{SO}}(L) = O^*(L) \cap \mathrm{SO}(L)$, and $\widetilde{\mathrm{SO}}^+(L) = O^+(L) \cap O^*(L) \cap \mathrm{SO}(L)$.
...
The **stable orthogonal group** $O^*(L)$ (also written $\widetilde{\mathrm{O}}(L)$) is the kernel of the natural map $O(L) \to O(A_L, q_L)$ to the isometry group of the discriminant form. Equivalently, it is the subgroup of $O(L)$ acting trivially on the discriminant group $A_L = L^*/L$.
...
> **Source**: Standard notion in quadratic form theory. For the correct discriminant-form approach to lattice classification, see [@Nik79].
...
- **[@dawes2021baily]**: M. Dawes, *The Baily-Borel compactification of a family of orthogonal modular varieties*, arXiv:2108.06236, 2021.  
  Extraction: `/home/dzack/research/theory/literature/dawes2021baily.md`

- **[@dawes2022orbits]**: M. Dawes, *Orbits in lattices*, arXiv:2205.10601, 2022.  
  Extraction: `/home/dzack/research/theory/literature/dawes2022orbits.md`
...
- **[@bailyborel1966]**: W. L. Baily Jr. and A. Borel, *Compactification of arithmetic quotients of bounded symmetric domains*, Ann. of Math. 84 (1966), 442–528.  
  Extraction: `/home/dzack/research/theory/literature/bailyborel1966.md`
...
- **[@conway1988mass]**: J. H. Conway and N. J. A. Sloane, *Low-dimensional lattices. IV. The mass formula*, Proc. Roy. Soc. London Ser. A 419 (1988), 259–286.  
  Extraction: `/home/dzack/research/theory/literature/conway1988mass.md`

- **[@CS99]**: J. H. Conway and N. J. A. Sloane, *Sphere Packings, Lattices and Groups*, 3rd ed., Grundlehren 290, Springer, 1999.  
  Extraction: `/home/dzack/research/theory/literature/conway1999sphere.md`
...
- **[@Huy16]**: D. Huybrechts, *Lectures on K3 Surfaces*, Cambridge Studies in Advanced Mathematics 158, Cambridge University Press, 2016.  
...
- **[@milnor1973symmetric]**: J. Milnor and D. Husemoller, *Symmetric Bilinear Forms*, Springer, 1973. (Milgram's formula appears in Appendix 4; cited as [MH] in Bruinier.)  
  Extraction: `/home/dzack/research/theory/literature/milnor1973symmetric.md`

- **[@Nik79]**: V. V. Nikulin, *Integer Symmetric Bilinear Forms and Some of Their Geometric Applications*, Izv. Akad. Nauk SSSR Ser. Mat. 43 (1979), no. 1, 111–177.  
  Extraction: `/home/dzack/research/theory/literature/nikulin1979integral.md`  
  Theorem numbers are citable directly, e.g. `[@Nik79, Theorem 1.14.2]`, `[@Nik79, Proposition 1.15.1]`, `[@Nik79, Corollary 1.12.3]`.

- **[@vinberg1972units]**: É. B. Vinberg, *The Groups of Units of Certain Quadratic Forms*, Mat. Sb. (N.S.) 87 (1972), no. 129, 18–36.  
  Extraction: `/home/dzack/research/theory/literature/vinberg1972units.md`

- **[@Vin75]**: É. B. Vinberg, *Some Arithmetical Discrete Groups in Lobachevskiĭ Spaces*, in: Discrete Subgroups of Lie Groups and Applications to Moduli, Oxford University Press, 1975, 323–348.  
  Extraction: `/home/dzack/research/theory/literature/vinberg1975arithmetical.md`

- **[@vinberg1983two]**: É. B. Vinberg, *The Two Most Algebraic K3 Surfaces*, Math. Ann. 265 (1983), no. 1, 1–21.  
  Extraction: `/home/dzack/research/theory/literature/vinberg1983two.md`
